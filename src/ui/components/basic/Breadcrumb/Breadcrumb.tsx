...
class Breadcrumb extends React.Component<BreadcrumbProperty, BreadcrumbState> {
  ...
  async checkAndCollapse() {
    await this.boxReady()
    let boxEl = this.props.box.current
  ...
  render() {
    return <div className="kiwi-breadcrumb">{this.state.fold ? <>
      ...
      menuProps={{
        items: items.map(it => {
          return {
            id: it.uri,
            text: it.name,
            onClick: this.props.onItemClick ? _ => { this.props.onItemClick(it) } : null
          }
        }),
        styles: {
          text: {
