...
class ContextualMenu extends React.Component<ContextualMenuProperty, {}> {
  render() {
    ...
    const style = Object.assign({}, this.props.styles?.root)
    return (
      <div className="kiwi-contextual-menu" style={style}>
        {this.props.items.map(item => {
          return (
            ...
            >
              {item.iconName && (
                <div className="kiwi-contextual-menu-item-icon">
                  <i className={`ms-Icon ms-Icon--${item.iconName}`} />
                </div>
              )}
              <div style={this.props.styles?.text} className="kiwi-contextual-menu-item-text">
                {item.text}
              </div>
            </button>
          )
